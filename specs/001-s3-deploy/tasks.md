...
**Checkpoint**: 執行 `npm run build`，確認 `frontend/dist/index.html` 中資源引用為 `/assets/...`（非 `/performance-dashboard/assets/...`）
...
**依賴**: T003（US3）必須先完成，確保 build 產物 base path 正確

### Implementation for User Story 1

- [ ] T004 [US1] 移除 `.github/workflows/deploy-frontend.yml` 中的 `peaceiris/actions-gh-pages@v4` deploy 步驟，並將 `permissions: contents: write` 改為 `contents: read`（FR-004）
- [ ] T005 [US1] 在 `.github/workflows/deploy-frontend.yml` 頂層新增 concurrency group：`group: deploy-frontend, cancel-in-progress: true`（FR-010）
- [ ] T006 [US1] 在 `.github/workflows/deploy-frontend.yml` 新增 "Deploy to S3" step：`aws s3 sync frontend/dist/ s3://${{ secrets.AWS_S3_BUCKET }}/ --delete`，並在 step env 注入 `AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`、`AWS_REGION`（FR-001、FR-002、FR-003、FR-011）
...
- [ ] T007 [US2] 確認 `.github/workflows/collect-metrics.yml` 的 `gh workflow run deploy-frontend.yml` trigger 機制在 S3 部署後仍有效（驗證任務，無程式碼修改，FR-008）
...
**Purpose**: 憲法修訂、文件同步、既有測試確認

- [ ] T008 [P] 更新 `.specify/memory/constitution.md`：修訂「技術限制與規範」中的 Vite base 規則，由「固定為 `/performance-dashboard/`」改為「由部署目標決定；S3 domain root 使用 `/`，GitHub Pages 子路徑使用對應前綴」，版本號升至 v1.3.0（plan.md Constitution Check justified violation）
- [ ] T009 [P] 更新 `PROJECT_SPEC.md` §8 CI/CD 流程：反映 deploy workflow 改用 `aws s3 sync`，移除 gh-pages 相關描述（憲法開發工作流程，CI/CD workflow 修改需更新）
- [ ] T010 執行既有測試確認無回歸：`cd scripts && uv run --with jira --with pyyaml --with python-dateutil --with pytest --with PyGithub pytest tests/ -v` 以及 `cd frontend && npm test`
- [ ] T011 依 `specs/001-s3-deploy/quickstart.md` 逐項執行手動驗收測試（SC-001～SC-004）
...
- **US1 (Phase 4)**: 依賴 T003（US3 build 產物 base 正確）
- **US2 (Phase 5)**: 依賴 US1 完成（S3 deploy 正常運作後才能驗證 trigger chain）
- **Polish (Phase 6)**: T008/T009 可在 US1 後立即並行；T010/T011 依賴全部實作完成
...
T003 [US3] vite.config.js
  ↓
T004 → T005 → T006 [US1] deploy-frontend.yml
  ↓
T007 [US2] 驗證 trigger chain
  ↓
T008 T009 [P] 文件更新
T010 T011 最終驗證
```
...
- T004、T005 雖在同一 workflow 檔案，但邏輯獨立，可分段處理後合併
- T008 和 T009 完全並行（不同文件）
- T010 和 T011 的 Python 測試與前端測試可並行
...
Task T004: "移除 gh-pages 步驟 + 更新 permissions"
  ↓
Task T005: "加入 concurrency group"
  ↓
Task T006: "加入 Deploy to S3 step"

# Polish 階段可並行：
Task T008: "更新 constitution.md"
Task T009: "更新 PROJECT_SPEC.md §8"
```
...
2. 完成 Phase 3（US3）：`vite.config.js` base → `/`
3. 完成 Phase 4（US1）：改寫 `deploy-frontend.yml`
4. **STOP and VALIDATE**：手動觸發 workflow，確認 S3 部署成功（SC-001、SC-003）
5. 驗證通過後進入 Phase 5、6

### Incremental Delivery

1. T003 (US3) + T004~T006 (US1) → 手動觸發 workflow 驗證 S3 基本部署 ✅
2. T007 (US2) → 驗證 daily trigger chain ✅
3. T008~T009 → 文件同步 ✅
4. T010~T011 → 最終驗收 ✅
...
- 憲法 v1.3.0 修訂（T008）需單獨 commit：`docs: amend constitution to v1.3.0（更新 Vite base 規則適配 S3 部署）`
