<!-- Sync Impact Report
版本變更：1.1.0 → 1.2.0
修訂原因：新增 commit 單一職責規則；強化 PROJECT_SPEC.md 更新觸發條件為「完成新功能需求」；修正原則 VI/VII 排序顛倒的文件錯誤。

新增/強化規則（2 條）：
  - 技術限制與規範：新增「Commit 單一職責」MUST 規則
  - 開發工作流程：PROJECT_SPEC.md 更新觸發條件從「改動程式邏輯後」強化為「完成新功能需求開發後」

修正問題（1 項）：
  - 原則 VI（向後相容 Schema 演進）與 VII（TDD）在文件中排序顛倒，已修正為正確順序 I→VII

範本一致性檢查：
  - .specify/templates/plan-template.md  ✅ 相容（Constitution Check 欄位無需修改）
  - .specify/templates/spec-template.md  ✅ 相容（無原則特定衝突）
  - .specify/templates/tasks-template.md ✅ 已更新（Notes 區塊新增 commit 單一職責提醒）
  - MEMORY.md（auto memory）           ✅ 已反映（工作流程提醒與憲法一致）
