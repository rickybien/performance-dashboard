<!-- Sync Impact Report
版本變更：1.2.0 → 1.3.0
修訂原因：部署目標從 GitHub Pages 遷移至 AWS S3（001-s3-deploy）；更新 Vite base 規則及相關描述。

修改規則（3 條）：
  - 技術限制與規範：Vite base 從「固定為 /performance-dashboard/，禁止修改」改為「設為 /，對應 S3 domain root」
  - 原則 I 原因：部署描述從 GitHub Pages 更新為 AWS S3
  - 原則 VI 原因：前端讀取描述從 GitHub Pages 更新為 AWS S3

範本一致性檢查：
  - .specify/templates/plan-template.md  ✅ 相容（Constitution Check 欄位無需修改）
...
**原因**：靜態部署於 AWS S3，零後端維運成本；24 小時更新精度已足夠工程指標使用場景。
...
**原因**：dashboard.json 由 CI 每日更新，前端從 AWS S3 讀取；
...
- **路由**：vue-router hash history，確保 S3 靜態部署相容性（S3 不支援 SPA server-side routing，hash history 可避免直接路徑刷新時的 404）。
- **Vite base**：設為 `/`，對應 S3 static website hosting 的 domain root 存取（`https://sit-kkday-performance-dashboard.s3.ap-southeast-1.amazonaws.com/`）。若未來改為 sub-path 部署，需同步修改此值。
...
- **Commit 單一職責**：每個 commit MUST 只做一件事；嚴禁在單一 commit 中混入不同目的的變更
  （例如：功能實作 + 重構 + 文件更新）。需要多種變更時，MUST 拆分為多個獨立 commit，各自描述其唯一目的。
- **檔案大小原則**：每個模組保持小而聚焦，一個 collector 對應一個資料來源。
- **型別標註**：Python 程式碼 MUST 使用 type hints；Vue 元件使用 `<script setup>` 隱含型別。

## 開發工作流程

每次完成新功能需求開發後，MUST 同步更新 `PROJECT_SPEC.md` 對應章節（程式邏輯變更亦適用）：
...
**Version**: 1.3.0 | **Ratified**: 2026-03-04 | **Last Amended**: 2026-03-04
